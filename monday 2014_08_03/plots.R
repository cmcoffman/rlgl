...
require(ggplot2)
cshl.plot.OD <- ggplot(cshl.all, aes(green.intensity, OD.600))
cshl.plot.OD=cshl.plot.OD +
  geom_point() +
  #scale_x_discrete(limits=c("mock","MeJA")) +
  facet_grid(.~plate) +
  ggtitle(label="OD vs Intensity for each plate") +
  xlab("Green Intensity") +
  ylab("OD.600")
cshl.plot.OD
ggsave("cshl-plot-OD.png")

cshl.plot.GFP <- ggplot(cshl.all, aes(green.intensity, GFP))
cshl.plot.GFP=cshl.plot.GFP +
  geom_point() +
  #scale_x_discrete(limits=c("mock","MeJA")) +
  facet_grid(.~plate) +
  ggtitle(label="GFP vs Intensity for each plate") +
  xlab("Green Intensity") +
  ylab("GFP") +
...
cshl.plot.GFP.row <- ggplot(cshl.all, aes(factor(row), GFP))
...
cshl.plot.OD.row <- ggplot(cshl.all, aes(factor(row), OD.600)) +
  geom_boxplot() +
  facet_grid(.~plate) +
...
cshl.plot.GFP.column <- ggplot(cshl.all, aes(factor(column), GFP)) +
  geom_boxplot() +
  facet_grid(.~plate) +
...
cshl.plot.OD.column <- ggplot(cshl.all, aes(factor(column), OD.600)) +
  geom_boxplot() +
  facet_grid(.~plate) +
...
cshl.plot.OD.vs.GFP <- ggplot(cshl.all, aes(GFP, OD.600)) +
  geom_point() +
  facet_grid(.~plate) +
